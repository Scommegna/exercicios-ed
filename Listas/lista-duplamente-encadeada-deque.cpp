...
acaoPrograma listadup::removeNoFim() {
    ...
    noh *aux = this->ultimo;
    acaoPrograma retorno = this->ultimo->acao;
...
void removeEspecifico(listadup &lista, string acao) {
    if(lista.vazia()) throw runtime_error("Remoção em lista vazia!");

    listadup aux = listadup();

    while(!lista.vazia()) {
        acaoPrograma aux2 = lista.removeNoFim();

        if(aux2.nomeAcao != acao) {
            aux.insereNoFim(aux2);
        }
    }

    while(!aux.vazia()) {
        lista.insereNoFim(aux.removeNoFim());
    }
}
    ...
    do {
        try {
            ...
            switch (comando) {
                ...
                case 'x':
                    cin >> acao;
                    removeEspecifico(minhaLista, acao);
                    break;
                case 'f': // finalizar
                    // checado no do-while
                    break;
