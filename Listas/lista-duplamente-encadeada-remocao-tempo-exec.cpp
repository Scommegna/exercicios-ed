...
public:
    ...
    void removeNoFim();
    void removeNoInicio();
    void retiraElementoMaiorTempoExec();
};
...
void listadup::retiraElementoMaiorTempoExec() {
    if(this->vazia()) throw runtime_error("Remoção em lista vazia!");
    ...
    while(aux != NULL) {
        if(aux->acao.tempoExecucao > aux2->acao.tempoExecucao) {
            aux2 = aux;
        }
    ...
    acaoPrograma dado = aux2->acao;

    if(this->primeiro->acao.identificador == aux2->acao.identificador) this->removeNoInicio();
    else if(this->ultimo->acao.identificador == aux2->acao.identificador) this->removeNoFim();
    else {
        aux = aux2->proximo;
        noh *aux3 = aux2->anterior;

        if(aux3 != NULL) aux3->proximo = aux2->proximo;
    
        if(aux != NULL) aux->anterior = aux2->anterior;
    ...
    cout << "(" << dado.identificador << ", " << dado.nomeAcao << ", " << dado.tempoExecucao << ", " << dado.tempoConsumido << ")" << endl;
}
        ...
        try {
            ...
            switch (comando) {
                ...
                case 'm': // inserir
                    ...
                    break;   
                case 't':
                    minhaLista.retiraElementoMaiorTempoExec();
                    break;          
                case 's': // procurar
                    cin >> nomeEquipe;
