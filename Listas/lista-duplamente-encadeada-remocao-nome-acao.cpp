...
void listadup::removePorAcao(string a) {
    if(this->vazia()) throw runtime_error("Remoção em lista vazia!");
    ...
    while(aux->acao.nomeAcao != a && aux != NULL) {
        aux2 = aux;
        aux = aux->proximo;
        counter++;

        if(aux->acao.nomeAcao == a) {
            encontrado = true;
        }
    }

    if(counter == 0 && encontrado) {
        this->removeNoInicio();
    } else if (counter == this->tamanho - 1) {
        this->removeNoFim();
    } else if(encontrado) {
    ...
    if(!encontrado) throw runtime_error("Erro: remoção de valor não encontrado!");
}
...
void listadup::removePorId(int id) {
    ...
    while(aux->acao.identificador != id && aux != NULL) {
        aux2 = aux;
        aux = aux->proximo;
        counter++;

        if(aux->acao.identificador == id) {
            encontrado = true;
        }
    }
    ...
    do {
        try {
            ...
            switch (comando) {
                ...
                case 'x':
                    cin >> nomeAcao;
                    minhaLista.removePorAcao(nomeAcao);
                    break;
                case 'w':
                    cin >> id;
